...
select brand from cars where length(brand)=5 group by brand;
-- #12 Отобразите общую сумму всех автомобилей марки Mercedes-Benz
select sum(price) as summ_Mercedes_Benz from cars where brand='Mercedes-Benz';
-- #13 Отобразите самую дорогую и самую дешевую машину
select price from cars order by price asc limit 1;
select price from cars order by price  desc limit 1;
...
-- #16 Отобразите 10 новейших машин с 5-го по 15-е. (Вопрос не понял, но решил как я понял по своему если правильно)
select brand,year_of_issue from cars order by year_of_issue desc offset 5 limit 15;
